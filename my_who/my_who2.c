...
{
	printf("%-15.15s\t%-15.15s\t%-15.14s\t", usuario , linea, str_tiempo);
	if(!globalArgs.salida_breve && globalArgs.muestra_actividad) printf("%-6.6s\t", actividad);
	printf("%-25.25s\t", comentario);
	if(!globalArgs.salida_breve) printf("%-12.12s", str_salida);
	printf("\n");
}
...
print_cabecera (void)
{
	print_linea(15,"NOMBRE",15,"LINEA","TIEMPO","INACT.","COMENTARIO",""/*vacio segun enunciado*/);
}
...
{
	if (globalArgs.incluye_login){  //devolvemos: id=ut_id
